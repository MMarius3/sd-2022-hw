...
//          "TRUNCATE `role`;",
  ...
  private static void bootstrapUserData() throws SQLException {
    for (String schema : SCHEMAS) {
      ...
      bootstrapRights();
      bootstrapRoleRight();
      bootstrapDefaultUsers(connectionWrapper);
      bootstrapUserRoles();
    }
  }
  ...
  private static void bootstrapDefaultUsers(JDBConnectionWrapper connectionWrapper){
    ...
    User admin = new UserBuilder().setUsername("admin@bank.co").setPassword(PasswordEncoder.encode("123")).setRoles(List.of(adminRole)).build();
    User employee = new UserBuilder().setUsername("e1@bank.co").setPassword(PasswordEncoder.encode("123")).setRoles(List.of(employeeRole)).build();
